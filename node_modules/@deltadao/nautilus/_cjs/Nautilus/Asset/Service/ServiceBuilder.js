...
const NautilusService_1 = require("./NautilusService");
class ServiceBuilder {
    service = new NautilusService_1.NautilusService();
    constructor(config) {
        if ('serviceType' in config) {
            this.service.type = config.serviceType;
            if (this.service.type === NautilusService_1.ServiceTypes.COMPUTE) {
            ...
            this.service.type = NautilusService_1.ServiceTypes[service.type.toUpperCase()];
            ...
            const datatokenObj = aquariusAsset.datatokens.find((datatoken) => datatoken.address === service.datatokenAddress);
            ...
            if (service.compute)
                this.service.compute = service.compute;
            if (service.name)
    ...
    addTrustedAlgorithms(trustedAlgorithmAssets) {
        ...
        for (const trustedAlgorithmAsset of trustedAlgorithmAssets) {
            const existingIndex = this.service.addedPublisherTrustedAlgorithms.findIndex((existingAsset) => existingAsset.did === trustedAlgorithmAsset.did);
            if (existingIndex > -1) {
    ...
    removeTrustedAlgorithm(did) {
        ...
        this.service.compute.publisherTrustedAlgorithms =
            this.service.compute.publisherTrustedAlgorithms.filter((algorithm) => algorithm.did !== did);
        return this;
    }
    ...
    addTrustedAlgorithmPublisher(publisherAddress) {
        ...
        if (!this.service.compute.publisherTrustedAlgorithmPublishers.includes(publisherAddress)) {
            this.service.compute.publisherTrustedAlgorithmPublishers.push(publisherAddress);
        }
        return this;
    }
    removeTrustedAlgorithmPublisher(publisherAddress) {
        ...
        this.service.compute.publisherTrustedAlgorithmPublishers =
            this.service.compute.publisherTrustedAlgorithmPublishers.filter((address) => address.toLowerCase() !== lowerCasePublisherAddress);
        return this;
    }
    ...
    setDatatokenNameAndSymbol(dtName, dtSymbol) {
        this.service.datatokenCreateParams = {
            ...this.service.datatokenCreateParams,
            name: dtName,
            symbol: dtSymbol
    ...
    setPricing(pricing) {
        if (this.service.editExistingService)
            throw new Error('Can not set new pricing configs for existing services using the builder. Use nautilus.setServicePrice() method instead.');
        this.service.pricing = pricing;
        return this;
    }
    reset() {
        this.service = new NautilusService_1.NautilusService();
    }
    build() {
        if (this.service.pricing === undefined &&
