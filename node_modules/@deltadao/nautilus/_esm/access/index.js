 ...
 */
export async function access(accessConfig) {
    ...
    const accessDetails = await getAccessDetails(config.subgraphUri, asset.datatokens[0].address, asset.services[0].timeout, signerAddress);
    ...
    const initializeData = await initializeProvider(assetWithAccessDetails, signerAddress, accessService, fileIndex, userdata);
    const assetWithPrice = await getAssetWithPrice(assetWithAccessDetails, signer, config, initializeData.providerFee);
    LoggerInstance.debug('[access] AssetWithprice:', assetWithPrice.orderPriceAndFees);
    if (isOwned(accessDetails)) {
        LoggerInstance.debug(`Found valid order for ${asset.id} with datatoken ${accessDetails.datatoken.address}`);
...
async function getAssetDownloadUrl(asset, signer, service, fileIndex, userCustomParameters) {
    ...
    const downloadUrl = await ProviderInstance.getDownloadUrl(asset.id, service.id, fileIndex || 0, asset.accessDetails.validOrderTx, service.serviceEndpoint, signer, userCustomParameters);
    return downloadUrl;
}
