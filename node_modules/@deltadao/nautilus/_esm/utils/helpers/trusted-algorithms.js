...
async function getPublisherTrustedAlgorithms(dids, metadataCacheUri) {
    ...
    for (const asset of assets) {
        ...
        const filesChecksum = await checkDidFiles(asset?.id, asset?.services?.[0]?.id, asset?.services?.[0]?.serviceEndpoint);
...
export async function resolvePublisherTrustedAlgorithms(nautilusDDOServices, metadataCacheUri) {
