import { Datatoken, Dispenser, FixedRateExchange, LoggerInstance, ZERO_ADDRESS, amountToUnits, approve } from '@oceanprotocol/lib';
import { getServiceById } from '.';
export async function isOrderable(asset, serviceId, algorithm, algorithmAsset) {
    const datasetService = getServiceById(asset, serviceId);
    if (!datasetService)
        return false;
    if (datasetService.type === 'compute') {
        ...
        if (algorithm.documentId) {
            const algoService = getServiceById(algorithmAsset, algorithm.serviceId);
...
export async function reuseOrder({ signer, asset, validOrderTx, providerFees }) {
    const datatoken = new Datatoken(signer);
    const tx = await datatoken.reuseOrder(asset.accessDetails.datatoken.address, validOrderTx, providerFees);
    return tx;
}
export async function order({ signer, asset, orderPriceAndFees, accountId, config, providerFees, computeConsumerAddress }) {
    const datatoken = new Datatoken(signer);
    const orderParams = {
    ...
    switch (asset.accessDetails?.type) {
        case 'fixed': {
            ...
            if (templateId === 1) {
                // buy datatoken
                const tx = await approve(signer, config, accountId, asset.accessDetails.baseToken.address, config.fixedRateExchangeAddress, await amountToUnits(signer, asset?.accessDetails?.baseToken?.address, orderPriceAndFees.price, asset?.accessDetails?.baseToken?.decimals), false);
                const txApprove = typeof tx !== 'number' ? await tx.wait() : tx;
                if (!txApprove) {
                    LoggerInstance.error('Unable to approve datatoken tx');
                    return;
                }
                const fre = new FixedRateExchange(config.fixedRateExchangeAddress, signer);
                const freTx = await fre.buyDatatokens(asset.accessDetails?.addressOrId, '1', orderPriceAndFees.price, ZERO_ADDRESS, '0');
                await freTx.wait();
            ...
            if (templateId === 2) {
                const tx = await approve(signer, config, accountId, asset.accessDetails.baseToken.address, asset.accessDetails.datatoken.address, await amountToUnits(signer, asset?.accessDetails?.baseToken?.address, orderPriceAndFees.price, asset?.accessDetails?.baseToken?.decimals), false);
                const txApprove = typeof tx !== 'number' ? await tx.wait() : tx;
                if (!txApprove) {
                    LoggerInstance.error('Unable to approve datatoken tx');
                    return;
        ...
        case 'free': {
            LoggerInstance.debug('[order] order with type "free" for templateId:', templateId);
            if (templateId === 1) {
                ...
                LoggerInstance.debug('[order] free order: dispenser tx', dispenserTx.hash);
                return await datatoken.startOrder(asset.accessDetails.datatoken.address, orderParams.consumer, orderParams.serviceIndex, orderParams._providerFee, orderParams._consumeMarketFee);
            }
            if (templateId === 2) {
