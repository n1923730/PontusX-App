import { ConfigHelper, LoggerInstance, Nft } from '@oceanprotocol/lib';
import { utils as ethersUtils } from 'ethers';
import { LifecycleStates } from '../@types';
import { access } from '../access';
import { compute, getStatus, retrieveResult, stopCompute } from '../compute';
import { createAsset, createServiceWithDatatokenAndPricing, publishDDO } from '../publish';
...
export class Nautilus {
    ...
    static async create(signer, config) {
        LoggerInstance.debug('Creating new Nautilus instance with signer', await signer.getAddress());
    ...
    async publish(asset) {
        ...
        // --------------------------------------------------
        await resolvePublisherTrustedAlgorithms(asset.ddo.services, chainConfig.metadataCacheUri);
        // --------------------------------------------------
        // 3. Create Datatokens and Pricing for new Services
        // --------------------------------------------------
        const services = [];
        for (const service of asset.ddo.services) {
            const serviceWithDatatokenAndPricing = await createServiceWithDatatokenAndPricing(service, signer, chainConfig, nftAddress, asset.owner);
    ...
    async edit(asset) {
        ...
        const services = [];
        await resolvePublisherTrustedAlgorithms(nautilusDDOServices, chainConfig.metadataCacheUri);
    ...
    async setServicePrice(aquaAsset, serviceId, newPrice) {
        ...
        return await editPrice(aquaAsset, serviceId, newPrice, this.config, this.signer);
    }
    ...
    async setAssetLifecycleState(aquariusAsset, state) {
        ...
        if (existingNftState === state) {
            LoggerInstance.warn(`[lifecycle] Asset lifecycle state is already ${state} (${LifecycleStates[state]}), action aborted`);
            return;
        }
        LoggerInstance.debug(`[lifecycle] Change asset lifecycle state from ${existingNftState} (${LifecycleStates[existingNftState]}) to ${state} (${LifecycleStates[state]}) `);
