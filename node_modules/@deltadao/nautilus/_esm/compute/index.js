...
import { getAssetWithPrice, getAssetsWithAccessDetails } from '../utils/helpers/assets';
import { isOrderable, order, reuseOrder } from '../utils/order';
import { approveProviderFee, initializeProviderForCompute, startComputeJob, stopComputeJob } from '../utils/provider';
export async function compute(computeConfig) {
    const { dataset: datasetConfig, // TODO consider syncing naming to prevent renaming
    algorithm: algorithmConfig, signer, chainConfig, additionalDatasets: additionalDatasetsConfig } = computeConfig;
    ...
    LoggerInstance.log('[compute] Starting compute order for dataset', datasetDid, '\nwith algorithm', algorithmDid, '\nfor account', signerAddress);
    ...
    try {
        ...
        const assets = await getAssetsWithAccessDetails(assetIdentifiers, chainConfig, signer);
        ...
        const providerInitializeResults = await initializeProviderForCompute(dataset, algo, signerAddress, computeEnv);
        ...
        const _algoDatatokenBalance = await getDatatokenBalance(signer, algo.services[0].datatokenAddress);
        ...
        // TODO remove? never used. maybe missing feature to check if datatoken already in wallet?
        const _datasetDatatokenBalance = await getDatatokenBalance(signer, algo.services[0].datatokenAddress);
        ...
        const response = await startComputeJob(dataset.services[0].serviceEndpoint, computeAsset, {
...
async function getComputeAssetPrices(algo, dataset, signer, config, providerInitializeResults) {
    ...
    const datasetWithPrice = await getAssetWithPrice(dataset, signer, config, datasetInitializeResult.providerFee);
    if (!datasetWithPrice?.orderPriceAndFees)
        throw new Error('Error setting dataset price and fees!');
    const algorithmWithPrice = await getAssetWithPrice(algo, signer, config, providerInitializeResults.algorithm.providerFee);
    if (!algorithmWithPrice?.orderPriceAndFees)
        throw new Error('Error setting algorithm price and fees!');
    return { datasetWithPrice, algorithmWithPrice };
}
export async function getStatus(computeStatusConfig) {
    ...
    try {
        const status = await ProviderInstance.computeStatus(providerUri, signerAddress, jobId);
...
export async function retrieveResult(computeResultConfig) {
    const { providerUri, signer, jobId, resultIndex } = computeResultConfig;
    const job = await getStatus(computeResultConfig);
    if (job?.status !== 70) {
        LoggerInstance.log('[compute] Retrieve results: job does not exist or is not yet finished.');
        return;
    }
    if (!job?.results || job.results.length < 1) {
        LoggerInstance.error('[compute] Retrieve results: could not find results for the job.');
        return;
    }
    const index = resultIndex ||
        job.results.indexOf(job.results.find((result) => result.type === 'output'));
    if (index < 0) {
        LoggerInstance.error('[compute] Retrieve results: resultIndex needs to be specified. No default output result found.', index);
        return;
    }
    LoggerInstance.debug('[compute] Build result url...');
    return await ProviderInstance.getComputeResultUrl(providerUri, signer, jobId, index);
}
export async function getComputeEnviroment(asset) {
...
export async function handleComputeOrder(signer, asset, orderPriceAndFees, accountId, initializeData, config, computeConsumerAddress) {
    LoggerInstance.debug('[compute] Handle compute order for asset type: ', asset.metadata.type);
    try {
        // Return early when valid order is found, and no provider fees
        // are to be paid
        if (initializeData?.validOrder && !initializeData?.providerFee) {
            LoggerInstance.debug('[compute] Has valid order: ', initializeData.validOrder);
        ...
        if (initializeData?.providerFee?.providerFeeAmount &&
            ...
            const txApproveProvider = await approveProviderFee(asset, accountId, signer, initializeData.providerFee.providerFeeAmount);
            if (!txApproveProvider)
                throw new Error('Failed to approve provider fees!');
            LoggerInstance.debug('[compute] Approved provider fees:', txApproveProvider);
        }
...
export async function stopCompute(stopComputeConfig) {
