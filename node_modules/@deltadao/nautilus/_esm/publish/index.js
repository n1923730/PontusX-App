import { Aquarius, Datatoken, LoggerInstance, Nft, NftFactory, ProviderInstance } from '@oceanprotocol/lib';
import { utils as ethersUtils } from 'ethers';
import { LifecycleStates } from '../@types';
export async function createAsset(assetConfig) {
    ...
    const nftFactory = new NftFactory(chainConfig.nftFactoryAddress, signer, chainConfig.network, chainConfig);
...
async function createDatatokenAndPricing(config) {
    ...
    switch (pricing.type) {
        case 'fixed':
            LoggerInstance.debug('[publish] Creating fixed rate exchange for datatoken...', { datatokenAddress, pricing });
    ...
    LoggerInstance.debug('[publish] Pricing scheme created.', pricingTransactionReceipt);
    const tx = await pricingTransactionReceipt.wait();
    return { datatokenAddress, tx };
}
export async function publishDDO(config) {
    const { chainConfig, signer, ddo, asset } = config;
    const publisherAccount = await signer?.getAddress();
    // --------------------------------------------------
    // 1. Validate DDO schema
    // --------------------------------------------------
    LoggerInstance.debug(`[publish] Validating DDO via ${chainConfig.metadataCacheUri}`);
    ...
    const encryptedDDO = await ProviderInstance.encrypt(ddo, chainConfig.chainId, chainConfig.providerUri);
    if (!encryptedDDO)
...
export async function createServiceWithDatatokenAndPricing(service, signer, chainConfig, nftAddress, assetOwner) {
